package cn.matrixsci.doublerecording.core.tts;
...
import cn.matrixsci.doublerecording.core.tts.control.InitConfig;
import cn.matrixsci.doublerecording.core.tts.control.MySyntherizer;
import cn.matrixsci.doublerecording.core.tts.control.NonBlockSyntherizer;
import cn.matrixsci.doublerecording.core.tts.listener.UiMessageListener;
import cn.matrixsci.doublerecording.core.tts.util.Auth;
import cn.matrixsci.doublerecording.core.tts.util.AutoCheck;
import cn.matrixsci.doublerecording.core.tts.util.IOfflineResourceConst;
import cn.matrixsci.doublerecording.core.tts.util.OfflineResource;
...
public class TtsProxy {
    ...
    private static volatile TtsProxy instance;
...
//    protected int descTextId = R.raw.sync_activity_description;
    ...
    public static TtsProxy getInstance(){
        if(instance==null){
            TtsProxy ttsProxy = new TtsProxy();
            ttsProxy.init();
            instance = ttsProxy;
        }
        return instance;
    ...
    protected void initialTts() {
        ...
        synthesizer = new NonBlockSyntherizer(AppContext.getInstance(), config, mainHandler); // 此处可以改为MySyntherizer 了解调用过程
    }
    ...
    protected InitConfig getInitConfig(SpeechSynthesizerListener listener) {
        ...
        // 上线时请删除AutoCheck的调用
        AutoCheck.getInstance(AppContext.getInstance()).check(initConfig, new Handler() {
            @Override
    ...
    protected OfflineResource createOfflineResource(String voiceType) {
        ...
        try {
            offlineResource = new OfflineResource(AppContext.getInstance(), voiceType);
        } catch (IOException e) {
            // IO 错误自行处理
            e.printStackTrace();
...
//    }
    ...
    private void checkResult(int result, String method) {
        if (result != 0) {
            LogUtils.e("error code :" + result + " method:" + method);
        }
    }
